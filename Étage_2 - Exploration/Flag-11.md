...
- On fait l'extraction de `N` et `e` de la clé public de l'un des deux `kristi82` ou `browndavid` puis une factorisation avec `f11_qp.py` pour trouver les deux facteur de `N = p x q` et on a le `d` bingo on peut générer  la clé privée
